...
export default function CompanyShowcasePage() {
    ...
    useEffect(() => {
        const fetchQuadras = async () => {
            setIsLoading(true)
            try {
                const response = await api.get(`/companies/${id}/courts`)
    ...
    // Filtrar quadras
    const quadrasFiltradas = courts.filter(
        (quadra) =>
            (tipoFiltro === "todos" || quadra.sportType.toLowerCase() === tipoFiltro.toLowerCase()) &&
            (quadra.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
                quadra.description?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                quadra.sportType.toLowerCase().includes(searchTerm.toLowerCase())),
    )

    const tiposQuadras = ["todos", ...Array.from(new Set(courts.map((q) => q.sportType.toLowerCase())))]
                        ...
                        {quadrasFiltradas.map((quadra) => (
                            <Card key={quadra.id} className="overflow-hidden hover:shadow-lg transition-shadow">
                                <div className="relative h-48">
                                    <Image
                                        src={quadra.photos.length > 0 ? quadra.photos[0] : "/placeholder.svg"}
                                        alt={quadra.name}
                                        fill
                                        className="object-cover"
                                        sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 33vw"
                                    />
                                    <Badge className="absolute top-2 right-2 bg-green-600">{quadra.sportType}</Badge>
                                </div>
                                <CardContent className="p-6">
                                    <h3 className="text-xl font-bold mb-2">{quadra.name}</h3>
                                    <p className="text-gray-600 mb-4 line-clamp-2">{quadra.description}</p>
                                    <div className="space-y-2 mb-4">
                                        <Clock className="h-4 w-4 mr-2" />
                                        <span>
                                            {quadra.openingTime} às {quadra.closingTime}
                                        </span>
                                    </div>
                                    <div className="flex items-center text-sm font-medium text-green-600">
                                        <span>R$ {quadra.hourlyPrice.toFixed(2)} / hora</span>
                                    </div>
