...
export default function CourtDetailsPage() {
  ...
  const [availableSlots, setAvailableSlots] = useState<
    { horario: string; disponivel: boolean }[]
  >([]);
    ...
    const fetchAvailableSlots = async () => {
      try {
        ...
        const response = await api.get(
          `/showcase/courts/${id}/available-slots?date=${new Date().toISOString()}`
        );
        const unavailableSlots = response.data.map(
          (slot: BookingApi) =>
            ({
        ...
        const availableSlots = generateAvailableHours(
          court,
          unavailableSlots,
          currentDayIndex
        );
  ...
  const getFormattedBookingText = (booking: Booking) => {
    return `${new Date(booking.startTime).toLocaleDateString(
      "pt-BR"
    )} • ${getTimeFromDateString(booking.startTime)} - ${getTimeFromDateString(
      booking.endTime
    )}`;
  };
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex items-center mt-2 space-x-2">
                        <Badge className="bg-blue-500 hover:bg-blue-600">
                          {strToTitle(court.sportType.replace("_", " "))}
                        </Badge>
                        <Badge
                          className={
                            court.isActive
                    ...
                    <div className="text-right">
                      <div className="text-2xl font-bold text-primary">
                        R$ {(court.hourlyPrice / 100).toFixed(2)}
                      </div>
                    ...
                    <TabsContent value="info" className="space-y-4">
                      <div>
                        ...
                        <p className="text-gray-700">
                          {court.description || "Nenhuma descrição disponível."}
                        </p>
                      </div>
                        ...
                        <div className="flex items-start">
                          ...
                          <div>
                            ...
                            <p className="text-gray-600">
                              {getTimeFromDateString(
                                court.courtSchedule![currentDayIndex]
                                  .openingTime
                              )}{" "}
                              às{" "}
                              {getTimeFromDateString(
                            ...
                            >
                              ...
                              {horario.isOpen && (
                                <p className="text-slate-600 mt-1">
                                  {getTimeFromDateString(horario.openingTime)}{" "}
                                  às{" "}
                                  {getTimeFromDateString(horario.closingTime)}
                                </p>
                              )}
                            </div>
                          ))}
                          ...
                          {availableSlots.map((slot, index) => (
                            ...
                            >
                              ...
                              <div className="text-xs mt-1">
                                {slot.disponivel ? "Disponível" : "Reservado"}
                              </div>
                            </div>
                          ))}
                        </div>
                      ...
                      {bookings.length > 0 ? (
                        <div className="space-y-4">
                                ...
                                <div className="mt-2 sm:mt-0 flex flex-col sm:items-end">
                                  {
                                    <div className="flex items-center">
                                      ...
                                      <span className="font-medium">
                                        R${" "}
                                        {getBookingTotalPrice(booking).toFixed(
                                          2
                                        )}
                                      </span>
                                    </div>
                          ...
                          <Button
                            ...
                            onClick={() =>
                              router.push("/admin/bookings?court=" + court.id)
                            }
                          >
                        ...
                        <div className="text-center py-12 border rounded-md">
                          ...
                          <Button
                            ...
                            onClick={() =>
                              router.push(
                                "/admin/bookings/nova?court=" + court.id
                              )
                            }
                          >
                            Criar nova reserva
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <Button
                    ...
                    onClick={() =>
                      router.push("/admin/bookings/new?court=" + court.id)
                    }
                  >
